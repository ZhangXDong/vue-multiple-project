...
const tclMaterialDeliveryMixin = {
    computed: {
        ...mapGetters([
            // Tcl
            'tclSchedulingInfo',
    ...
    methods: {
        ...
        tclMaterialDelivery (information) {
            ...
            // 到达上料点
            if (curUrl && curUrl.indexOf('/tclMaterialDelivery/loadPoint') > -1) { // ?baseStatus=5&logicStatus=3 
                ...
                if (this.$route.path !== '/tclMaterialDelivery/loadPoint') {
                    this.$router.replace({ path: '/tclMaterialDelivery/loadPoint' })
                }
            }
            // 到达卸料点
            if (curUrl && curUrl.indexOf('/tclMaterialDelivery/unloadPoint') > -1) { // ?baseStatus=5&logicStatus=3
                ...
                if (this.$route.path !== '/tclMaterialDelivery/unloadPoint') {
                    this.$router.replace({ path: '/tclMaterialDelivery/unloadPoint' })
                }
            }
            // 到达退回物料卸料点（上料点）
            if (curUrl && curUrl.indexOf('/tclMaterialDelivery/waitDump') > -1) { // ?baseStatus=5&logicStatus=3
                this.saveTclGuideTip(`请将被退回货架推离车辆`)
                this.saveFooterBtns([{
                    title: '卸车完成',
                    disable: false,
                    type: 43,
                ...
                if (this.$route.path !== '/tclMaterialDelivery/waitDump') {
                    this.$router.replace({ path: '/tclMaterialDelivery/waitDump' })
                }
            }
        }
    }
...
export default tclMaterialDeliveryMixin
