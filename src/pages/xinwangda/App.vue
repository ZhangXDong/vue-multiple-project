...
import sunwodaGarbageCarryMixin from './mixin/sunwodaGarbageCarry.js'
import sunwodaMaterialCarryMixin from './mixin/sunwodaMaterialCarry.js'
...
export default {
		...
		sunwodaGarbageCarryMixin, 
		sunwodaMaterialCarryMixin 
		],
	data() {
		return {
			websocketData: {
	...
	mounted() {
		...
		//用于监听浏览器的刷新事件 userInfo信息持久化
		if(localStorage.getItem('userInfo')) {
			this.saveUserInfo(localStorage.getItem('userInfo'))
		}
	},
	methods: {
		...mapMutations({
			...
			saveUserInfo: 'SET_USERINFO' // 用户信息
		}),
	}
}
</script>
